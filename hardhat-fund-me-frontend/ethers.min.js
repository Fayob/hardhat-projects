...
function createGetUrl(options) {
  async function getUrl(req, _signal) {
    assert(
      _signal == null || !_signal.cancelled,
...
class JsonRpcApiProvider extends AbstractProvider {
  ...
  getRpcTransaction(tx) {
    ...
    if (tx.blobVersionedHashes) {
      result["blobVersionedHashes"] = tx.blobVersionedHashes.map((h) =>
        h.toLowerCase()
      );
    }
    return result;
...
class AnkrProvider extends JsonRpcProvider {
  ...
  static getRequest(network, apiKey) {
    ...
    const request = new FetchRequest(
      `https:/\/${getHost$5(network.name)}/${apiKey}`
    );
...
class AlchemyProvider extends JsonRpcProvider {
  ...
  static getRequest(network, apiKey) {
    ...
    const request = new FetchRequest(
      `https:/\/${getHost$4(network.name)}/v2/${apiKey}`
    );
    request.allowGzip = true;
...
class ChainstackProvider extends JsonRpcProvider {
  ...
  static getRequest(network, apiKey) {
    ...
    const request = new FetchRequest(
      `https:/\/${getHost$3(network.name)}/${apiKey}`
    );
    request.allowGzip = true;
